document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkWinConditionsAndProceed() {
        ...
        // KONDISI 2: Kemenangan Tim Undercover / Mr. White (FIXED LOGIC)
        // Menang jika jumlah (Undercover + Mr. White) sama dengan atau lebih banyak dari Civilian.
        else if ((activeUndercovers.length + activeMrWhites.length) >= activeCivilians.length) {
            winDetected = true;
        ...
        if (winDetected) {
            ...
            // This case handles the announcement after the last civilian is eliminated to trigger the win.
            if (eliminatedThisRoundPlayer?.originalRole === 'Civilian' && activeCivilians.length === 0) {
                 ...
                 resetTruthOrDareButtons();
                 eliminationResultPopup.classList.remove('hidden');
                 // The checkWinConditions will be called again after this popup is closed, leading to the winner announcement.
                 return;
            }
