document.addEventListener('DOMContentLoaded', async () => { // MODIFIED: Added 'async' keyword here
    // --- SECTIONS ---
    ...
    const closeForgotPasswordModalBtn = forgotPasswordModal.querySelector('.close-popup-btn');
    ...
    // MODIFIED: Handle password reset from URL with a stronger check
    ...
    if (typeParam === 'recovery' && accessToken) {
        ...
        // This will be handled after the initial auth state is checked
    }
    ...
    const openAuthModal = () => {
        ...
        forgotPasswordModal.classList.add('hidden'); // Ensure forgot password is hidden
        updatePasswordModal.classList.add('hidden'); // Ensure update password is hidden
    };
    ...
    if(sendResetLinkBtn) {
        sendResetLinkBtn.addEventListener('click', async () => {
            ...
            const { error } = await supabaseClient.auth.resetPasswordForEmail(email, {
                // MODIFIED: Ensure redirectTo includes path and a clear type/access_token
                redirectTo: window.location.origin + window.location.pathname + '?type=recovery&access_token=' 
            });
    ...
    // MODIFIED: Enhanced onAuthStateChange to handle password reset flow more reliably
    supabaseClient.auth.onAuthStateChange((event, session) => {
        // If we are in a password reset flow AND the session indicates signed_in,
        // it means Supabase has processed the token. We want to ensure
        // the update password modal stays visible, not hide it like normal login.
        if (isPasswordResetFlow && event === 'SIGNED_IN') {
            // Keep update password modal open and handle user data
            ...
            loadWordPacks(); // Load word packs for the user
            
            // Crucially, prevent the standard UI changes that would hide the reset modal
            authModal.classList.add('hidden'); // Ensure standard auth modal is hidden
            // No return here, let the other parts update for user data if needed, but not hide updatePasswordModal
        } else {
            // Standard auth state change logic
            ...
            if (user) {
                ...
                wordPackOwnerSpan.classList.remove('hidden');
                authModal.classList.add('hidden'); // Hide auth modal for regular login
                authBtn.onclick = handleLogout;
            } else {
                currentUser = null;
                customWordPacks = {}; 
    ...
    // MODIFIED: Ensured this button also clears the isPasswordResetFlow flag
    if(updatePasswordModal.querySelector('.close-popup-btn')) {
        updatePasswordModal.querySelector('.close-popup-btn').addEventListener('click', () => {
            ...
            // Potentially re-evaluate auth state or go to login after closing
            // if the user hasn't successfully updated password and clicked "Simpan & Login"
            openAuthModal(); 
        });
    }
    // --- END OF AUTH LOGIC ---
