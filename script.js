document.addEventListener('DOMContentLoaded', () => {
    ...
    const updatePasswordModal = document.getElementById('update-password-modal'); // NEW
    ...
    const showForgotPasswordLink = document.getElementById('show-forgot-password-link'); // NEW
    const forgotPasswordEmailInput = document.getElementById('forgot-password-email'); // NEW
    const sendResetLinkBtn = document.getElementById('send-reset-link-btn'); // NEW
    const forgotPasswordErrorP = document.getElementById('forgot-password-error'); // NEW
    const closeForgotPasswordModalBtn = forgotPasswordModal.querySelector('.close-popup-btn'); // NEW
    const updatePasswordInput = document.getElementById('update-password-input'); // NEW
    const updatePasswordBtn = document.getElementById('update-password-btn'); // NEW
    const updatePasswordErrorP = document.getElementById('update-password-error'); // NEW
    ...
    const resetPasswordToken = urlParams.get('access_token');
    const resetPasswordTrigger = urlParams.get('reset_password'); // Ini dari redirectTo di atas

    if (resetPasswordToken && resetPasswordTrigger === 'true') {
        authModal.classList.add('hidden');
        forgotPasswordModal.classList.add('hidden');
        updatePasswordModal.classList.remove('hidden');
        history.replaceState(null, '', window.location.origin + window.location.pathname); // Clean the URL
    }
    ...
    const clearAuthErrors = () => {
        ...
        forgotPasswordErrorP.classList.add('hidden'); // NEW
        updatePasswordErrorP.classList.add('hidden'); // NEW
    };
    ...
    const openAuthModal = () => {
        ...
        clearAuthErrors();
        loginView.classList.remove('hidden'); // Ensure login view is shown by default
        registerView.classList.add('hidden'); // Ensure register view is hidden
    };
    ...
    if(sendResetLinkBtn) {
        sendResetLinkBtn.addEventListener('click', async () => {
            ...
            forgotPasswordErrorP.classList.add('hidden');
            ...
            const { error } = await supabaseClient.auth.resetPasswordForEmail(email, {
                redirectTo: window.location.origin + '?reset_password=true' // URL to redirect after clicking email link
            });
    ...
    // NEW: Handle update password
    if(updatePasswordBtn) {
        updatePasswordBtn.addEventListener('click', async () => {
            playClickSound();
            const newPassword = updatePasswordInput.value.trim();
            updatePasswordErrorP.classList.add('hidden');
            ...
            if (error) {
                ...
                document.getElementById('goToLoginFromUpdateBtn').addEventListener('click', () => {
                    playClickSound();
                    updatePasswordModal.classList.add('hidden');
                    openAuthModal(); // Or show login view directly
                });
            }
        });
    }
    ...
    // NEW: Event listener for "Lupa Password?" link
    ...
    // NEW: Close button for forgot password modal
    ...
    // NEW: Close button for update password modal
    if(updatePasswordModal.querySelector('.close-popup-btn')) {
    ...
    function resetGameFull() {
        ...
        const allScreens = [playerNameInputSection, gameSection, mrWhiteGuessSection, eliminationResultPopup, winnerPopupModal, roundOverSection, finalScoreDisplayModal, cardRevealModal, musicSettingsModal, peekPlayerSelectModal, massPenaltyModal, howToPlayModal, wordPackManagerModal, authModal, forgotPasswordModal, updatePasswordModal]; // NEW
    ...
    function handleMrWhiteGuess() {
        ...
        if (isMrWhiteFinalDuel) {
            ...
            if (guess.toLowerCase() === civilianWord.toLowerCase()) {
                ...
                eliminatedThisRoundPlayer = civilianPlayer; 
                eliminatedThisRoundPlayer.isEliminated = true;
                eliminatedPlayerInfoH3.innerHTML = `<i class="fas fa-times-circle"></i> ${civilianPlayer.name} (Civilian) gagal menghentikan Mr. White!`;
            } else {
                playWinSound();
                currentWinningTeamType = "CIVILIAN_TEAM";
                currentWinnerPlayerObjects = players.filter(p => p.originalRole === 'Civilian');
                eliminatedThisRoundPlayer = mrWhitePlayer;
                eliminatedThisRoundPlayer.isEliminated = true;
                eliminatedPlayerInfoH3.innerHTML = `<i class="fas fa-times-circle"></i> ${mrWhitePlayer.name} (Mr. White) gagal menebak di duel terakhir!`;
            }
            resetTruthOrDareButtons();
            eliminationResultPopup.classList.remove('hidden');
    ...
    function checkWinConditionsAndProceed() {
        ...
        if (activePlayers.length === 2 && activeCivilians.length === 1 && activeUndercovers.length === 1) {
            ...
            document.getElementById('voting-phase').classList.add('hidden');
            eliminatedPlayerInfoH3.innerHTML = `<i class="fas fa-skull-crossbones"></i> ${losingCivilian.name} (Civilian) kalah dalam duel terakhir!`;
            resetTruthOrDareButtons();
            eliminationResultPopup.classList.remove('hidden');
