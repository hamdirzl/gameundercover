document.addEventListener('DOMContentLoaded', () => {
    ...
    let isMrWhiteFinalDuel = false; // --- PENANDA BARU --- untuk duel Mr. White
    ...
    function resetGameFull() {
        ...
        isMrWhiteFinalDuel = false; // --- PERUBAHAN BARU ---
    ...
    // --- PERUBAHAN BESAR: Logika tebakan Mr. White dirombak untuk menangani duel ---
    function handleMrWhiteGuess() {
        ...
        gameInProgress = false; // Permainan pasti berakhir setelah tebakan ini

        if (isMrWhiteFinalDuel) {
            // Logika untuk duel 1 vs 1
    ...
    function checkWinConditionsAndProceed() {
        ...
        // --- PERUBAHAN BARU: Duel 1 Civilian vs 1 Mr. White ---
        if (activePlayers.length === 2 && activeCivilians.length === 1 && activeMrWhite.length === 1) {
            gameInProgress = false; // Permainan dihentikan untuk tebakan terakhir
            isMrWhiteFinalDuel = true; // Tandai ini adalah duel terakhir
            document.getElementById('voting-phase').classList.add('hidden');
            mrWhiteGuessSection.classList.remove('hidden');
            mrWhiteNameGuessStrong.textContent = activeMrWhite[0].name;
            // Ubah teks di popup tebakan untuk mencerminkan duel
            ...
            return; // Hentikan fungsi untuk menunggu tebakan
        }
    ...
    // --- PERUBAHAN BARU: Logika penentuan pemenang dirombak untuk eksklusivitas Mr. White ---
    function displayRoundOverDetails() {
        ...
        players.forEach(p => {
            ...
            // --- Logika Penentuan Pemenang yang Diperbarui ---
            let isWinner = false;
            if (currentWinningTeamType === "MR_WHITE_GUESS") {
                // Kemenangan solo Mr. White, hanya cek objek pemainnya
                isWinner = currentWinnerPlayerObjects.some(wp => wp.name === p.name);
            } else {
                // Logika lama untuk kemenangan tim
                const winningRoles = [];
    ...
    function calculateAndDisplayRoundScores() {
        players.forEach(player => {
            ...
            // Untuk kemenangan tim Civilian, semua anggota tim dapat poin.
