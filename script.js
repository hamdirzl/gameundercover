document.addEventListener('DOMContentLoaded', () => {
    ...
    function playMusic(src) { backgroundMusic.src = src; const playWhenReady = () => { if (audioContext && audioContext.state !== 'running') return; const playPromise = backgroundMusic.play(); if (playPromise !== undefined) { playPromise.then(() => { isMusicPlaying = true; musicManuallyPaused = false; toggleMusicBtn.querySelector('i').className = 'fas fa-volume-mute'; }).catch(error => { isMusicPlaying = false; musicManuallyPaused = true; toggleMusicBtn.querySelector('i').className = 'fas fa-music'; console.error("Gagal memutar musik:", error); }); } backgroundMusic.removeEventListener('canplaythrough', playWhenReady); }; backgroundMusic.addEventListener('canplaythrough', playWhenReady, { once: true }); }
    ...
    if (toggleMusicBtn && backgroundMusic) { toggleMusicBtn.addEventListener('click', () => { playClickSound(); resumeAudioContext(); if (!isMusicPlaying) { musicManuallyPaused = false; backgroundMusic.play().then(() => { isMusicPlaying = true; toggleMusicBtn.querySelector('i').className = 'fas fa-volume-mute'; }).catch(() => { isMusicPlaying = false; musicManuallyPaused = true; toggleMusicBtn.querySelector('i').className = 'fas fa-exclamation-triangle'; }); } else { backgroundMusic.pause(); isMusicPlaying = false; musicManuallyPaused = true; toggleMusicBtn.querySelector('i').className = 'fas fa-music'; } }); }

    // --- EVENT LISTENERS ---
    showGuideBtn.addEventListener('click', (e) => { e.preventDefault(); playClickSound(); howToPlayModal.classList.remove('hidden'); });
    closeGuideBtn.addEventListener('click', () => { playClickSound(); howToPlayModal.classList.add('hidden'); });
    openMusicSettingsBtn.addEventListener('click', (e) => { e.preventDefault(); playClickSound(); musicSettingsModal.classList.remove('hidden'); });
    ...
    function resetGameFull() {
        ...
        if (isMusicPlaying) { backgroundMusic.pause(); isMusicPlaying = false; musicManuallyPaused = true; toggleMusicBtn.querySelector('i').className = 'fas fa-music'; }
    ...
    function validateConfigAndProceed() {
        ...
        if (!isMusicPlaying && musicManuallyPaused) {
            ...
            if (playPromise !== undefined) { playPromise.then(() => { isMusicPlaying = true; toggleMusicBtn.querySelector('i').className = 'fas fa-volume-mute'; }).catch(error => { console.error("Pemutaran otomatis gagal:", error); musicManuallyPaused = true; isMusicPlaying = false; toggleMusicBtn.querySelector('i').className = 'fas fa-music'; }); }
        }
