document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeParam === 'recovery' && accessToken) {
        ...
        // Optionally focus the new password input
        updatePasswordInput.focus();
    }
    ...
    supabaseClient.auth.onAuthStateChange((event, session) => {
        // Special handling for recovery event:
        // If the user is signed in due to a recovery link AND the update password modal is visible,
        // we keep the update password modal visible and don't try to hide authModal.
        if (event === 'SIGNED_IN' && updatePasswordModal && !updatePasswordModal.classList.contains('hidden')) {
            ...
            // Do NOT hide authModal or show initialConfigSection here
            authBtn.onclick = handleLogout;
            loadWordPacks(); // Load word packs for the user who is now technically signed in
            return; // Exit to prevent standard sign-in logic from interfering
        }
    ...
    function generateRolesToDistribute() { rolesToDistribute = []; for (let i = 0; i < initialCiviliansCount; i++) rolesToDistribute.push("Civilian"); for (let i = 0; i < initialUndercoversCount; i++) rolesToDistribute.push("Undercover"); for (let i = 0; i < initialMrWhitesCount; i++) rolesToDistribute.push("Mr. White"); for (let i = rolesToDistribute.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1));[rolesToDistribute[i], rolesToDistribute[j]] = [rolesToDistribute[j], roles[i]]; } }
