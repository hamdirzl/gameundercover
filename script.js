document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================================
    // =========== CORE WIN CONDITION LOGIC (FIXED & IMPROVED) ============
    // =========================================================
    function checkWinConditionsAndProceed() {
        ...
        // Hitung jumlah tim "jahat" yang masih aktif.
        const evilTeamCount = activeUndercovers.length + activeMrWhites.length;
        ...
        // Ini terjadi jika tidak ada lagi Undercover DAN Mr. White yang tersisa.
        if (evilTeamCount === 0) {
            winDetected = true;
            determinedWinType = "CIVILIAN_TEAM";
            determinedWinners = players.filter(p => p.originalRole === 'Civilian');
        }
        // KONDISI 2 (REVISI): Kemenangan Tim Undercover / Mr. White
        // Ini terjadi jika jumlah tim jahat SAMA DENGAN atau LEBIH BANYAK dari Civilian.
        // Ini secara otomatis mencakup kasus 1 vs 1, dan juga kasus di mana Civilian habis (evilTeamCount >= 0).
        else if (evilTeamCount >= activeCivilians.length) {
            winDetected = true;
        ...
        if (winDetected) {
            ...
            // Logika ini untuk memastikan pop-up eliminasi pemain terakhir tetap muncul sebelum layar pemenang
            if (eliminatedThisRoundPlayer?.originalRole === 'Civilian' && activeCivilians.length === 0) {
                 ...
                 resetTruthOrDareButtons();
                 eliminationResultPopup.classList.remove('hidden');
                 // Kita return di sini agar pop-up pemenang muncul SETELAH pop-up eliminasi ditutup
                 return; 
            }
